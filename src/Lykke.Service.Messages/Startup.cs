...
using Lykke.Service.Messages.Modules;
...
namespace Lykke.Service.Messages
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            {
                options.DefaultLykkeConfiguration("v1", "Template Messages API");
            });
            ...
            builder.RegisterModule(new ServiceModule(appSettings.TemplateMessageSettings, log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            ...
            app.UseLykkeMiddleware("Template Messages", ex => new {Message = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, AppSettings settings)
        {
            ...
            var dbLogConnectionString = settings.TemplateMessageSettings.Db.LogsConnString;

            // Creating azure storage logger, which logs own messages to concole log
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                const string appName = "Lykke.Service.TemplateMessage";

                var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                    appName,
                    new AzureTableStorage<LogEntity>(dbLogConnectionString, "TemplateMessageLog", consoleLogger),
                    consoleLogger);
